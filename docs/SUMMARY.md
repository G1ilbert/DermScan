...
## Worker Deployment & Model Path Fix

The DermScan worker (the polling consumer of the `scans` table) was
not deployed to Railway. The API service was the only one running,
which meant every `POST /scan` would insert a `pending` row that
nothing ever picked up. Submitted scans appeared to "hang" forever.

### What was missing

- **Worker service.** Railway only had `DermScan` (the API). No
  worker meant `app.worker.tasks` never ran in production. The polling
  loop, the `SELECT ... FOR UPDATE SKIP LOCKED` claim, and the ONNX
  inference path were all only available locally.
- **ONNX weights inside the image.** Even on the API side, the
  Dockerfile didn't include `/models/dermscan.onnx`. `MODEL_PATH`
  defaulted to `/models/dermscan.onnx`, the file wasn't there, and
  the worker's `_model_available()` would have returned `False` —
  meaning `run_inference` would have fallen through to its mock
  branch on every request.
- **`COPY models/ /models/` wouldn't have fixed it.** Three reasons:
  Docker build context is `./backend`, so the host `models/` is out
  of scope; `models/*.onnx` and `*.onnx.data` are gitignored, so
  Railway's fresh-clone build wouldn't see them; and Railway was
  initially on Railpack which ignores the Dockerfile entirely.

### What was fixed

- **`backend/Dockerfile`** now downloads the model at build time via
  `curl` from the public GitHub Release v1.3.0, pinned with
  `ARG MODEL_RELEASE_TAG`. Build log confirmed the 72.84 MB
  `dermscan.onnx.data` and the 1.20 MB `dermscan.onnx` landed in
  `/models/`. ([`68034c5`](https://github.com/G1ilbert/DermScan/commit/68034c5))
- **Railway switched to the Dockerfile builder** via the
  `backend/railway.json` `builder: DOCKERFILE` directive that was
  already in the repo. The build log showed Railway running
  `FROM python:3.11-slim` through to the `curl ... | -o
  /models/dermscan.onnx.data` step exactly as written.
- **New Railway service `fantastic-vibrancy`** created from the same
  `G1ilbert/DermScan` repo with **Root Directory = `backend`** and
  **Start Command = `python -m app.worker.tasks`**. All 13 env vars
  copied from the API via the Raw Editor's Copy ENV → OS clipboard →
  Ctrl+V flow, so the secret values never landed in chat
  transcripts.
- **Healthcheck path removed from `backend/railway.json`.** Both
  services share that config (same Root Directory), and the worker
  has no HTTP server — Railway was looping `Attempt #N failed with
  service unavailable` on `GET /health`. Dropped
  `healthcheckPath: "/health"` entirely; both services now rely on
  Railway's container-level liveness check.
  ([`daac4bb`](https://github.com/G1ilbert/DermScan/commit/daac4bb))

### Current status

Both services on Railway are `Online`. Verified end-to-end:

- `curl https://dermscan-production.up.railway.app/health` →
  `200 {"success":true,"data":{"status":"ok"},"error":null}`
- Worker deploy logs show:
  - `Polling worker starting — interval=2.0s stuck_reset=300s`
  - `_reset_stuck` ran on startup (UPDATE scans SET
    status='pending' WHERE status='processing' AND updated_at <
    cutoff)
  - Polling SELECT firing every ~2s as configured:
    `SELECT scans.id FROM scans WHERE status='pending' ORDER BY
    created_at ASC LIMIT 1 FOR UPDATE SKIP LOCKED`

The `MODEL_PATH=/models/dermscan.onnx` env var resolves to the file
baked in at build time, so the worker now performs real ONNX
inference (5-pass TTA, sigmoid average, confidence-gated band)
instead of returning the deterministic mock prediction.

### Trade-off taken

Dropping `healthcheckPath` from `railway.json` means the API loses
its automated deploy-gate (Railway no longer waits for `GET /health`
to return 200 before swapping traffic to a fresh container). The
container-level liveness check still catches crashed processes. For
a single-service deploy this is a real regression worth flagging; if
the API ever ships a bug that returns 200 from `/health` but crashes
on real requests, that bad version will get traffic. The right
follow-up is to give each service its own config file once the
project grows to need it.
